...
class GameMap(val width: Short,
  ...
  def posIsOutsideMap(pos: Position): Boolean = {
    pos match {
      case x if (x.xPos < 0) => true
      case x if (x.yPos < 0) => true
      case x if (x.xPos > width) => true
      case x if (x.yPos > height) => true
      case _ => false
    }
  }
  ...
  def nearbyEntitiesByDistance(entity: Entity): Map[Double, Entity] = {
    (planets.values
      ...
      ++
        allShips
          .filter(_ != entity)
          .map(ship => entity.getDistanceTo(ship) -> ship)
          .toMap[Double, Entity])
  }

  def nearbyEnemiesByDistance(entity: Entity): Map[Double, Ship] = {
    allShips
      .filter(_.owner!= Some(playerId))
